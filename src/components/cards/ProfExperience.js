...
const ProfExperience = ({
  ...
  const handleDelete = () => {
    if (user) {
      ...
      axios.post(`${baseURL}/deletedExperience`, {
        headers: {
      ...
      <div className="expBox-card">
        ...
        <div className="exp-boxTwo">
          <div className="exp-boxThree">
            ...
            {/* <p className="expDuration">{`${expDuration.split("-")[0]} ${months[(Number(expDuration.split("-")[1]))-1]}`}</p> */}
            <p className="expDuration">{expDuration}</p>
          </div>
          <p className="expDese">{expDesc}</p>
        </div>
        <div id="edit-btn-div">
          <button onClick={handleDelete} id="exp-delete-btn" className="exp-deleteBtn" type="button">
            Delete
          </button>
        </div>
      </div>
